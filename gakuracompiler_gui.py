...
class GsEditor:
	...
	def __init__(self):
		...
		mt["Insert"].add_command(label="define macro/マクロ定義",command=lambda:self.inscode(';KEY is always upper. KEYは大文字。\n;"[[KEY]]"is "value"\n@define KEY value'))
		...
		mt["Insert"].add_command(label="javascript",command=lambda:self.inscode('@eval code'))
		mt["Insert"].add_command(label="function/関数",command=lambda:self.inscode('@function my_function(args){code}'))
